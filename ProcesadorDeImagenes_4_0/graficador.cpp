...
void Graficador::resizeGL(int w, int h)
{
    ...
    glViewport(0,0,w,h);//todo lo dibujado se arma sobre este plano, los parametros w yh son la posicion que va a tener la ventana respecto de donde se pone el resultado
    glMatrixMode(GL_PROJECTION);//a partir de esto, todo lo que esta debajo afecta la proyeccion
    glLoadIdentity();
    ...
    glOrtho(xmin,w+xmax,ymin,h+ymax,zmin,zmax);//elijo que parte del plano quiero ver
    glMatrixMode(GL_MODELVIEW);// afecta el modelo
    glLoadIdentity();
...
void Graficador::paintGL()
{
    ...
    relacionAncho=(float)width()/(float)imagen.getAncho();//ancho y altura del viewport
    relacionAlto=(float)height()/(float)imagen.getAlto();//obtengo una relacion
    ...
    {
        escala=relacionAncho;//se toma como escala la menor de las relaciones

        dy=(height()-(float)(imagen.getAlto()*escala))*0.5f;//se centra la imagen
    }
    else
    {
        escala=relacionAlto;
        dx=(width()-(float)(imagen.getAncho()*escala))*0.5f;//se centra la imagen
    }
...
void Graficador::DibujarHistograma()
{
    ...
    Hist_G=stat.Hist_G();
    Hist_B=stat.Hist_B();
    Hist_intensidades=stat.Hist_intensidad();
  ...
  glClear( GL_COLOR_BUFFER_BIT );// limpia la pantalla
      ...
      glMatrixMode(GL_PROJECTION);//establece la proyeccion
      glLoadIdentity();
      glOrtho(0,imagen.getM(),0,stat.getMF_intensidad().second/2,-1,1);
      glMatrixMode(GL_MODELVIEW);//se afecta el modelo
      glLoadIdentity();
   ...
   for(auto mapaR: Hist_R)
   {

       glColor3f(1,0,0);

       glVertex2f(mapaR.first,mapaR.second);
   }
   glEnd();

  glBegin(GL_LINE_STRIP);
   for(auto mapaG: Hist_G)
   {

       glColor3f(0,1,0);

       glVertex2f(mapaG.first,mapaG.second);
   }
   glEnd();

   glBegin(GL_LINE_STRIP);

   for(auto mapaB: Hist_B)
   {

       glColor3f(0,0,1);

       glVertex2f(mapaB.first,mapaB.second);
   }
   glEnd();

     glBegin(GL_LINE_STRIP);

   for(auto mapaInt: Hist_intensidades)
   {

       glColor3f(1,1,0);

       glVertex2f(mapaInt.first,mapaInt.second);
   }
    ...
    if(ctrl_and_m)
    {
     ...
     if(ctrl_and_h)
     {
        ...
        else
        {
         ...
         repaint();
          resizeGL(imagen.getAncho(),imagen.getAlto());
...
void Graficador::DibujarImagen()
{
    ...
    glTranslatef(dx,dy,0.0f);//se centra
    glScalef(escala,escala,1.0f);//se escala
